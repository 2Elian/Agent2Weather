...
from a2w.smw.agents import ForecastWeatherAgent, SummaryAgent, SuggestionAgent, BriefAgent
...
class TestDB:
    def __init__(self, db: SQLServerConnector):
        ...
        self.brief_instance = BriefAgent(llm=self.llm_instance, config=smw_config)
    @classmethod
    ...
    @time_recorder(func_name="BriefTester")
    async def brief(self, regions: List[str], start_date: str, end_date: str) -> Optional[Dict[str, Any]]:
        state = await self.suggest(regions=regions, start_date=start_date, end_date=end_date)
        state = await self.summary_instance.run(state)
        result = await self.brief_instance.run(state)
        return result
