...
class TestDB:
    def __init__(self, db: SQLServerConnector):
        ...
        print(smw_config)
        self.wr_instance = WeatherReportWorkflow(llm=self.llm_instance, db_connector=self.db, config=smw_config)
    @classmethod
    ...
    @time_recorder(func_name="WrTester")
    async def wr(self, regions: List[str], start_date: str, end_date: str):
        state = {
            "task_type": "气象呈阅件",
        ...
        result = await self.wr_instance.run(state)
        return result
